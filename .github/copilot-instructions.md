...
**UMCP v2.2.5** · **15,014 tests** · **20 domains** · **181 closure modules** · **47 lemmas** · **44 structural identities**
...
pytest                                       # 15,014 tests (pytest --collect-only | grep ":" | wc -l to verify)
...
**15,014 test cases** across **175 test files** in `tests/` (174 top-level `test_*.py` + 1 in `tests/closures/` + `conftest.py`), numbered by tier and domain (`test_000_*` through `test_293_*`). Single `tests/conftest.py` provides:
...
- Parametrized tests expand the collected items to 15,014 (verify: `pytest --collect-only | grep "::" | wc -l`)
