...
**UMCP v2.3.0** · **19,631 tests** · **21 domains** · **222 closure modules** · **47 lemmas** · **44 structural identities**
...
pytest                                       # 19,631 tests (pytest --collect-only | grep ":" | wc -l to verify)
...
**19,631 test cases** across **224 test files** in `tests/` (223 top-level `test_*.py` + 1 in `tests/closures/` + `conftest.py`), numbered by tier and domain (`test_000_*` through `test_325_*`). Single `tests/conftest.py` provides:
...
- Parametrized tests expand the collected items to 19,631 (verify: `pytest --collect-only | grep "::" | wc -l`)
