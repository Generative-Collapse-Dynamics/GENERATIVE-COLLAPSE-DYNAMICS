...
**UMCP v2.3.0** · **19,157 tests** · **20 domains** · **220 closure modules** · **47 lemmas** · **44 structural identities**
...
pytest                                       # 19,157 tests (pytest --collect-only | grep ":" | wc -l to verify)
...
**19,157 test cases** across **216 test files** in `tests/` (215 top-level `test_*.py` + 1 in `tests/closures/` + `conftest.py`), numbered by tier and domain (`test_000_*` through `test_325_*`). Single `tests/conftest.py` provides:
...
- Parametrized tests expand the collected items to 19,157 (verify: `pytest --collect-only | grep "::" | wc -l`)
