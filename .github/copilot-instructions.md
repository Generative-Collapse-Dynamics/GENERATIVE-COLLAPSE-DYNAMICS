...
**UMCP v2.3.0** · **15,748 tests** · **20 domains** · **193 closure modules** · **47 lemmas** · **44 structural identities**
...
pytest                                       # 15,748 tests (pytest --collect-only | grep ":" | wc -l to verify)
...
**15,748 test cases** across **183 test files** in `tests/` (182 top-level `test_*.py` + 1 in `tests/closures/` + `conftest.py`), numbered by tier and domain (`test_000_*` through `test_293_*`). Single `tests/conftest.py` provides:
...
- Parametrized tests expand the collected items to 15,748 (verify: `pytest --collect-only | grep "::" | wc -l`)
