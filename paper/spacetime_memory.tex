...
This paper makes five structural claims about the accumulation
pattern.  Each is verified computationally by the theorem suite
(Sec.~\ref{sec:howto}) and each can be falsified by a single
counterexample:
\begin{enumerate}
...
\item \textbf{Structural consistency across all scales}
  ...
  $\IC = e^{\kappa}$), confirming that the diagnostic
  framework is self-consistent across the entire
  scale ladder.
\end{enumerate}
...
The introduction made five claims.  We close the loop:
...
\item \textbf{Space accumulates as iterated $|\kappa|$}
  ...
  \emph{Falsified by}: an entity whose well depth does not
  grow linearly with iteration count.  (4/4 sub-tests.)
...
\item \textbf{Time emerges from return cost}
  ...
  \emph{Falsified by}: an entity whose independently measured
  temporal ordering contradicts $\mathcal{T}$.
  (3/3 + 3/3 sub-tests.)
...
\item \textbf{Memory accumulates as well geometry}
  ...
  mechanism.
  \emph{Falsified by}: a phase boundary where geometric
  slaughter fails to explain the $\IC/F$ drop.

\item \textbf{Budget gradient structures the manifold}
  (Theorem~\ref{thm:T1}).
  $\dd\Gam/\dd\omega > 0$ at 100~grid points and all
  37~positions; minimum $\Gam'(0.01) = 3.0 \times 10^{-4}$.
  \emph{Falsified by}: any $\omega$ where
...
\item \textbf{Structural consistency across all scales}
  ...
  \emph{Falsified by}: a single entity violating any
  identity.  (6/6 sub-tests.)
\end{enumerate}
